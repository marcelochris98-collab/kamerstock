...
<body class="bg-slate-100 min-h-screen">
    ...
    <div class="bg-slate-900 text-white px-4 py-4 sticky top-0 z-40">
        <div class="max-w-5xl mx-auto flex items-center justify-between gap-3">
            <div class="min-w-0">
                <p class="text-sm font-bold truncate">KamerStock</p>
                <p class="text-xs text-slate-400 truncate">Portail client</p>
            </div>

            @if(session('portal_client_id'))
            <form method="POST" action="{{ route('client.portal.logout') }}">
                @csrf
                <button class="text-xs px-3 py-2 bg-white/10 rounded-lg hover:bg-white/20 whitespace-nowrap">
                    Déconnexion
                </button>
            </form>
            @endif
    ...
    <div class="bg-white border-b border-slate-200 sticky top-[64px] z-30">
        <div class="max-w-5xl mx-auto px-3 sm:px-4 py-3 flex gap-2 overflow-x-auto">
